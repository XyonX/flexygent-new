...
def agent_loop(conversation:Conversation,input_message:str,tools:list,tool_registry:ToolRegistry,client,config:AgentConfig):
    ...
    response = client.chat.completions.create(model="openrouter/owl-alpha",messages=conversation.to_dict(),tools=tools)

    iter_no=1
    while iter_no<=config.max_iteration and response.choices[0].finish_reason !="stop":
        # inject warning 
        # if iter_no==config.max_iteration-1:
